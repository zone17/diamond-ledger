...
import Foundation
// The pure ASR value layer (`Transcript`, `TranscriberEngine`, `ConfidenceMapping`) lives in
// `SpeechTypes` (DL-37) so `Parse` / the headless `dl-score` CLI can depend on it without the
// iOS-only engine code. `@_exported` keeps existing `import DiamondSpeech` consumers resolving
// `Transcript` / `TranscriberEngine` unchanged.
...
// `Transcript`, `TranscriberEngine`, and `ConfidenceMapping` moved to the `SpeechTypes`
// target (DL-37) — see the `@_exported import SpeechTypes` above. They are re-exported, so
// references here and in `import DiamondSpeech` consumers are unchanged.
