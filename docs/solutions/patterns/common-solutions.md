...
| 12 | A loose multi-signal guard (bare `contains`) in a safety-critical classifier → silent wrong judgment; use subject-anchored patterns + exclusion contexts, prefer `outOfGrammar` over a confident guess, and write ADVERSARIAL tests (tokens present but the play is NOT X) | P1 | [logic-errors/loose-substring-guard-silent-misclassification](../logic-errors/loose-substring-guard-silent-misclassification.md) |
