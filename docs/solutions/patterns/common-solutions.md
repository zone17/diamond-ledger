...
| 12 | **Fail-safe defaults at the probabilistic→deterministic seam (Art. VII):** a loose `contains` guard OR an unmeasured ASR confidence that defaults at/above a clarify threshold both produce a silent wrong judgment. The default/unmeasured/ambiguous case must fail toward surfacing-to-human, never confident-and-silent. Use anchored patterns, default confidence BELOW the gate, no unconditional "enhancement" override, and ADVERSARIAL tests + a structural pin (`assert default < threshold`) | P1 | [logic-errors/loose-substring-guard-silent-misclassification](../logic-errors/loose-substring-guard-silent-misclassification.md) |
| 13 | A derived export/report re-reading an append-only log must apply the SAME withhold/override rules as the canonical projection (skip open-judgment plays, substitute corrections) — reuse the code path, don't re-loop raw events, or the official artifact silently diverges from the source of truth. + cwevent export-conformance gotchas (segfaults: missing `start` records, `YYYY-MM-DD` date, error-advance double-encoding) | P1 | [best-practices/derived-export-must-match-canonical-projection](../best-practices/derived-export-must-match-canonical-projection.md) |
| 14 | **Red harness ≠ new bug:** a regression gate/harness built on a branch cut from a STALE base reports the missing upstream fix as a fresh regression (DL-37: every play rendered `6-3` because the base predated the #162 FactBridge fix; rebase → green). Triage a new gate's first red as base-staleness FIRST; never freeze a baseline from output you got on an unconfirmed base (you'd bless the bug). The mirror of "green CI ≠ correct." Also: a real end-to-end harness over the production seam earns its keep in minutes. | P2 | [workflow-issues/harness-on-stale-base-flags-missing-upstream-fix](../workflow-issues/harness-on-stale-base-flags-missing-upstream-fix.md) |
| 15 | **Eval-gate construction pitfalls:** a CI runner that (a) interpolates program output into comparator SOURCE (`"""${ACTUAL}"""` in a Python heredoc) corrupts on a quote/backslash — the exact regression inputs it must catch — and (b) SKIPs with `exit 0` indistinguishably from PASS makes a "hard gate" protect nothing. Pass output as DATA (file/stdin/argv); skip only where the gate genuinely can't run and hard-fail where it should; assert N>0 units compared; let a real regression exit non-zero WITH a legible banner (`cmd \|\| RC=$?`). | P1 | [best-practices/eval-gate-construction-pitfalls](../best-practices/eval-gate-construction-pitfalls.md) |
